...
describe('sqlite storage backend', () => {
	it('boots with defaults and persists observations', async () => {
		...
		const storage = createStorage({ backend: 'sqlite', sqlitePath: dbPath }, 'rook');
	...
	it('supports task and runtime policy flows', async () => {
		...
		const usage = await storage.getAgentRuntimeUsage('rook', new Date(Date.now() - 60_000).toISOString());
