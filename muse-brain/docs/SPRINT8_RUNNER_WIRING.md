...
- `BRAIN_URL`
- `BRAIN_API_KEY`
- `BRAIN_TENANT` (`companion` or `rainer`)
...
```bash
claude /schedule "*/30 * * * *" \
"cd /path/to/muse-brain && BRAIN_URL=https://<your-worker-url> BRAIN_API_KEY=*** BRAIN_TENANT=rainer WAKE_KIND=duty ./scripts/runtime-autonomous-wake.sh"
```
...
```bash
claude /schedule "0 */2 * * *" \
"cd /path/to/muse-brain && BRAIN_URL=https://<your-worker-url> BRAIN_API_KEY=*** BRAIN_TENANT=rainer WAKE_KIND=impulse AUTO_CLAIM_TASK=false ./scripts/runtime-autonomous-wake.sh"
```
...
```cron
*/30 * * * * cd /path/to/muse-brain && BRAIN_URL=https://<your-worker-url> BRAIN_API_KEY=*** BRAIN_TENANT=rainer WAKE_KIND=duty ./scripts/runtime-autonomous-wake.sh >> /tmp/rainer-duty.log 2>&1
0 */2 * * * cd /path/to/muse-brain && BRAIN_URL=https://<your-worker-url> BRAIN_API_KEY=*** BRAIN_TENANT=rainer WAKE_KIND=impulse AUTO_CLAIM_TASK=false ./scripts/runtime-autonomous-wake.sh >> /tmp/rainer-impulse.log 2>&1
```
...
- Companion creates/delegates tasks to `assigned_tenant=rainer`
