...
```text
$HOME/.claude/agents/memory/<agent>/_universal.md
```
...
```bash
mkdir -p "$HOME/.claude/agents/memory/michael"
cd "$HOME/.claude/agents/memory"
...
```json
"command": "AGENT_MEMORY_AUTOPUSH=1 python3 <home-dir>/.claude/hooks/agent-memory-harvester.py"
```
